...
scriptpath = "C:\Python27\posterStemmer.py"
...
#
con = sqlite3.connect("C:\Python27\indexer_part2.db")
...
cur.execute("create table if not exists Posting (TermId int, DocId int)")
...
for k in database.keys():
    cur.execute('insert into TermDictionary values (?,?)', (k, database[k].termid))
#
# Notice that each 'key' to the docids structure is in fact a docid
#
for i in database[k].docids.keys():
    cur.execute('insert into Posting values (?, ?)', (database[k].termid, i))
#
